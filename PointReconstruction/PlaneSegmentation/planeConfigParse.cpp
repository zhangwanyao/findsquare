...
planeSegConfigParse::planeSegConfigParse()
{
	this->freeMemory();
	sys_control_para_.is_cad = false;
	sys_control_para_.scanner_type = INVALID_SCAN_TYPE;

	/*config_params_.downsample_type = INVALID_DOWNSAMPLE_TYPE;
	config_params_.downsample_threshold = 200;
	config_params_.downsample_leaf_size = 0;
	config_params_.is_input_filtered = false;
	config_params_.is_input_unit_check = false;
	config_params_.origin_in_cloud_center = false;*/
...
void planeSegConfigParse::SyncPlaneSegConfigure(SegCntrlParams &config_params)
{
	...
	if ((sync_con) && (tmp_i < SEG_CONFIG_PARAM_MAX) && (tmp_i >= 0)) config_params.seg_thrshld_section_valid = tmp_i;// (tmp_i == 0) ? false : true;


}


//get plane segmentation control parameters from config section SEG_THRSHLD_CONFIG_DEBUG
void planeSegConfigParse::SyncPlaneSegConfigure(unsigned int option_section, float voxel_length, PlaneFitThresholds& plane_seg_th)
{
	int tmp_i = std::numeric_limits<int>::lowest();
...
void planeSegConfigParse::SyncPlaneTestConfigure(NormalEstimationCntrlParam& normalEst_control_params)
{
	int tmp_i = std::numeric_limits<int>::lowest();
	float tmp_f = std::numeric_limits<float>::infinity();
...
void planeSegConfigParse::SyncPlaneTestConfigure(FilterCntrlParams& filter_control_params)
{
	int tmp_i = std::numeric_limits<int>::lowest();
	float tmp_f = std::numeric_limits<float>::infinity();
...
void planeSegConfigParse::SyncPlaneSegConfigure(const std::string config_path,DebugConfigParams &DebugConfigParams)
{
	int tmp_i = std::numeric_limits<int>::lowest();
	bool sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "dbg_section_valid", tmp_i);
	if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.dbg_section_valid = (tmp_i == 0) ? false : true;

	if (DebugConfigParams.dbg_section_valid)
	{
		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "reserved_test", tmp_i);
		if ((sync_con) && (tmp_i >= 0)) DebugConfigParams.reserved_test = tmp_i;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "missing_point_output", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.missing_point_output = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "io_debug_info", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.io_debug_info = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "voxel_info_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.voxel_info_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "neighbour_info_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.neighbour_info_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "no_plane_merge", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.no_plane_merge = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "plane_output_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.plane_output_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "merge_pseudobad_voxel_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.merge_pseudobad_voxel_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "merge_bad_voxel_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.merge_bad_voxel_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "pseudo_bad_lost_plane_output_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.pseudo_bad_lost_plane_output_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "is_dwnsmpl_output", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.is_dwnsmpl_output = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "points_group_neighbor_output_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.points_group_neighbor_output_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "merge_same_plane_output_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.merge_same_plane_output_debug = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "include_get_nm_by_file", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.include_get_nm_by_file = (tmp_i == 0) ? false : true;

		sync_con = ReadConfigInt("SEG_DEBUG_CONFIG", "nm_file_ouput_debug", tmp_i);
		if ((sync_con) && (tmp_i < 2) && (tmp_i >= 0)) DebugConfigParams.nm_file_ouput_debug = (tmp_i == 0) ? false : true;
	...
	debug_config_params_ = DebugConfigParams;
}

#endif


bool planeSegConfigParse::GetPlaneSegConfigure(const std::string config_path)
{
...
bool planeSegConfigParse::GetPlaneTestConfigure(const std::string config_path)
{
	if (!this->GetConfigMap(config_path))
	{
		log_error("GetConfigMap failed");
...
//save plane segmentation config parameters  into file configParseInfo.txt for debug 
void planeSegConfigParse::SavePlaneSegConfigParams(const std::string file_name)
{
...
void planeSegConfigParse::SavePlaneTestConfigParams(const std::string file_name)
{
