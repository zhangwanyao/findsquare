...
#include "point_set_with_planes.h"
                ...
                */
                ...
                void Hypothesis<Kernel>::refine_planes() {
                        ...
                        for (std::size_t i = 0; i < segments.size(); ++i) {
                                ...
                                const Plane* plane = s->supporting_plane()? s->supporting_plane():s->fit_supporting_plane(); // user may provide invalid plane fitting (we always fit)
...
#define MERGED_
                ...
                void Hypothesis<Kernel>::construct_proxy_mesh(const Polygon_mesh& bbox_mesh, Polygon_mesh& candidate_faces) {
						...
						// The supporting planar index of each face
						typename Polygon_mesh::template Property_map<Face_descriptor, std::size_t> face_supporting_index
							= candidate_faces.template add_property_map<Face_descriptor, std::size_t>("f:supp_index").first;
                ...
                std::vector<typename Hypothesis<Kernel>::Face_descriptor>
						...
						// The supporting planar index of each face
						typename Polygon_mesh::template Property_map<Face_descriptor, std::size_t> face_supporting_index =
								 mesh.template property_map<Face_descriptor, std::size_t>("f:supp_index").first;

                        Planar_segment* supporting_segment = face_supporting_segments[face];
						std::size_t index = face_supporting_index[face];
						...
						face_supporting_index[f1] = index;
						...
						face_supporting_index[f2] = index;
