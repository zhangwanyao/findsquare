...
DW_Detc_Edge2D::DW_Detc_Edge2D(Point3fArray* ptArray, const Point3f nPlane, const double voxel_length)
	: m_ptArray(nullptr)	// init m_ptArray nullptr
	, m_voxelgrid(nullptr)	// init m_voxelgrid nullptr
	, m_pts(nullptr)
	, data_center_input(false)
...
bool DW_Detc_Edge2D::init() 
{
	// data validation
	edge_pts_in_projected_plane.clear();
	edge_pts_in_projected_plane.shrink_to_fit();
	if (!m_ptArray) {
		log_error("error DW_Detc_Edge2D::init->m_ptArray nullptr\n");
		
		return false;
	}else {
		if (m_ptArray->size() == 0) {
			log_error("error DW_Detc_Edge2D::init->invalid m_ptArray size\n");
	...
	if (Util_Math::vec3_is_zero(m_nPlane)) {
		...
		log_error("error DW_Detc_Edge2D::init->invalid m_nPlane\n");
		return false;
	}
...
#pragma omp parallel for
	...
	// create voxelgrid
	VoxelGrid2D* voxelgrid = new VoxelGrid2D;
	...
	if (!voxelgrid->setLenVoxel(vl)) {
		log_error("error DW_Detc_Edge2D::init -> setLenVoxel invalid\n");
		
		return false;
	}
	if (!voxelgrid->setExtent(extent_x, extent_y)) {
		
		log_error("error DW_Detc_Edge2D::init -> setExtent invalid\n");
		return false;
	}
	
	// initialize voxelgrid
	// init image
	initImgVoxelGrid(extent_y, extent_x); 
	//m_img_voxelgrid is 8UC1 image mask with dimension of extent_y x extent_x, indicating occupied or point intensity
...
bool DW_Detc_Edge2D::initImgVoxelGrid(unsigned int row, unsigned int col) {
	if (row && col) {
...
void DW_Detc_Edge2D::clearVoxelGrid() {
	if (m_voxelgrid) {
		m_voxelgrid->clear();
...
void DW_Detc_Edge2D::clearContours() {
...
bool DW_Detc_Edge2D::detect() {
	detect_img_cnt();
	return true;
}

bool DW_Detc_Edge2D::detect_bn() {
	detect_pt_edge_by_neighbor_voxels();
	return true;
}
static int cntA = 0;
void DW_Detc_Edge2D::detect_img_cnt() {
	...
	findContours(dst_thres, m_cnts, hierarchy, RETR_TREE, CHAIN_APPROX_NONE);
#if 0	
	CMat dst = src.clone();
	cout << "m_cnts_" << cntA <<" :"<< m_cnts.size()<<endl;
	drawContours(dst, m_cnts, -1, (0, 255, 255), 1);
	std::string file_name = output_path+ std::to_string(cntA++) + "voxelgrid_edge.jpg";
	imwrite(file_name, dst);
#endif
}

void DW_Detc_Edge2D::detect_img_edge() {
	using namespace cv;
	CMat src, src_gray;
...
void DW_Detc_Edge2D::detect_img_edge_Canny(CMat src_gray, 
...
void DW_Detc_Edge2D::detect_pt_edge_by_intensity() {
	...
	Voxel2D** voxels = m_voxelgrid->getVoxels();
...
void DW_Detc_Edge2D::detect_pt_edge_by_neighbor_voxels() {
	Voxel2D** voxels = m_voxelgrid->getVoxels();
	...
	{
		std::vector<std::vector<unsigned int>> tmp1,tmp2;
		pts_in_edge_voxels.swap(tmp1);
		pts_in_edge_voxels_neighbors.swap(tmp2);
	}
	/********* Left lateral ************/
...
void DW_Detc_Edge2D::collect_edge_pts_and_neighbor_pts(Voxel2D** voxels,
...
void DW_Detc_Edge2D::find_edge_pts_by_bilateral_kernal(const std::vector<std::vector<unsigned int>>& pts_in_edge_voxels, 
...
void DW_Detc_Edge2D::setPtArray(Point3fArray* ptArray) {
	m_ptArray = ptArray;
}

void DW_Detc_Edge2D::setNPlane(const Point3f nPlane) {
	m_nPlane = nPlane;
	// normalization
...
Point3fArray* DW_Detc_Edge2D::getPtArray() {
	return m_ptArray;
}

Point3f DW_Detc_Edge2D::getNPlane() {
	return m_nPlane;
}

VoxelGrid2D* DW_Detc_Edge2D::getVoxelGrid() {
	return m_voxelgrid;
}

CMat DW_Detc_Edge2D::getImgVoxelGrid() {
	return m_img_voxelgrid;
}

vector<vector<Point2i>> DW_Detc_Edge2D::getContours() {
	return m_cnts;
}

bool DW_Detc_Edge2D::setVoxelSize(double voxel_length) {
	voxel_size = voxel_length;
	return true;
}
double DW_Detc_Edge2D::getVoxelSize() {
	return voxel_size;
}

bool DW_Detc_Edge2D::get2DContours(Vector<Point2fArray> &contours)
{
	//vector<vector<Point3f>> project_contours;
	contours.clear();
	//contours.resize(m_cnts.size());
	Voxel2D **voxel_2d = m_voxelgrid->getVoxels();
...
bool DW_Detc_Edge2D::getOrigMapOfContours(vector<Point3fArray> &orig_map)
{
...
bool DW_Detc_Edge2D::get2DContours_bn(Point2fArray& contours)
{
	contours.clear();
...
bool DW_Detc_Edge2D::getOrigMapOfContours(ModuleStruct::Point3fArray& orig_map)
{
	Point3f nz(0.0f, 0.0f, 1.0f);
	bool isNeedRot = !Util_Math::vec3_are_same(m_nPlane, nz); // is need rotation
	CMat rot_mat(3, 3, CV_32F);
