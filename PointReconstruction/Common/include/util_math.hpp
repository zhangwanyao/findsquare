...
namespace Util_Math {
	...
	*/
	...
	inline float vec3_angle_deg(VEC vec0, VEC vec1) {
		vec0 = vec3_normalize(vec0);
		vec1 = vec3_normalize(vec1);
		float dotproduct = vec0.x * vec1.x + vec0.y * vec1.y + vec0.z * vec1.z;
		if (abs(dotproduct + 1e-6) > 1.f)
			dotproduct = 1.f - 1e-6;
		return acosf(dotproduct) / 3.1415926 * 180.0f;
	}
	...
	* @return  3*3 rotation matrix  */
	...
	//	input = PT(
	//		rot_mat.at<DT>(0, 0) * input.x + rot_mat.at<DT>(0, 1) * input.y + rot_mat.at<DT>(0, 2) * input.z,
	//		rot_mat.at<DT>(1, 0) * input.x + rot_mat.at<DT>(1, 1) * input.y + rot_mat.at<DT>(1, 2) * input.z,
	//		rot_mat.at<DT>(2, 0) * input.x + rot_mat.at<DT>(2, 1) * input.y + rot_mat.at<DT>(2, 2) * input.z);
	//}

	template<typename PT, typename T>
	static inline void rot(PT &input, const T rot_mat[3][3]) {
		input = PT(
			rot_mat[0][0] * input.x + rot_mat[0][1] * input.y + rot_mat[0][2] * input.z,
			rot_mat[1][0] * input.x + rot_mat[1][1] * input.y + rot_mat[1][2] * input.z,
			rot_mat[2][0] * input.x + rot_mat[2][1] * input.y + rot_mat[2][2] * input.z);
	}
...
#endif
	template<typename T>
	static inline float DWComputePointToPointDist(const T point_1, const T point_2) {
		return static_cast<float>(sqrt(pow(point_1.x - point_2.x, 2.0)
			+ pow(point_1.y - point_2.y, 2.0)
			+ pow(point_1.z - point_2.z, 2.0)));
	}
	...
	template<typename T>
	static inline float DWComputePointToPointDist2D(const T point_1, const T point_2) {
		return static_cast<float>(sqrt(pow(point_1.x - point_2.x, 2.0) + pow(point_1.y - point_2.y, 2.0)));
	}
	...
	template<typename OUT, typename T>
	static inline OUT ComputePointToLinesegDist2D(const T pt, const T ptA, const T ptB, bool onLineOnly)
	{
		cv::Point2d ptAP = ptA - pt;
		cv::Point2d ptAB = ptA - ptB;
		...
		OUT dist = std::sqrt((pt.x - px) * (pt.x - px) + (py - pt.y) * (py - pt.y));
		return dist;
	}
	...
	template<typename T>
	static inline float DWComputePointToPlaneDist(const T pt, const T plane_normals, const T plane_center) {
		return static_cast<float>(fabs(plane_normals.x * (pt.x - plane_center.x) +
			plane_normals.y * (pt.y - plane_center.y) +
			plane_normals.z * (pt.z - plane_center.z)));
		return static_cast<float>(pt.x + plane_normals.x);
	}

	template<typename OUT, typename T>
	static inline OUT CalLine2PlaneAngle(const T L_A, const T L_B, const T n) {
		T d = L_B - L_A;
		OUT angle = std::asinf(ComputeVectorDotProduct<OUT,T>(d, n) / vec3_len(d) / vec3_len(n));

		return static_cast<OUT>(angle * 180.f / CV_PI);
	}
	...
	template <typename T>
	static inline int LUDecomposition(const std::vector<std::vector<T>>& para, std::vector <T>& result)
	{
		const int equ = para.size(); // number of equation
		const int var = para[0].size(); //number of variable
		std::vector<std::vector<T>> L(equ, std::vector<T>(equ));
		std::vector<std::vector<T>> U(equ, std::vector<T>(equ));
		std::vector<T> y(equ);
		int i, j, k;
		int n = para.size();
		T t;
		...
		for (i = 0; i < n; i++)
		{
			...
			for (k = i + 1; k < n; k++)
			{
				...
				for (j = 0; j < i; j++)
					t = t + L[k][j] * U[j][i];
				t = para[k][i] - t;
				if (t != 0 && U[i][i] == 0)//ÅÐ¶Ï·ÖÄ¸ÊÇ·ñÎªÁã
					return 0;
				L[k][i] = t / U[i][i];
			}
			/*Çó½âU¾ØÕó*/
	  ...
	  //  {
	   //  printf(".8lf\n",y[i]);
	   // }
		 /*Í¨¹ý»Ø´ú·¨Çó½âÏßÐÔ·½³Ì×éUX=Y£¬²¢½«½á¹û·ÅÈëb[]ÖÐ*/
		for (i = n - 1; i >= 0; i--)
		{
			t = 0;
			for (j = i + 1; j < n; j++)
...
*/
	...
	inline bool isIdxInList(const T0 idx, const std::vector<T1> &list) {
