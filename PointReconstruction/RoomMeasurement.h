...
{
	// Legacy internal modes:
	// ROOMCONTOUR: directly square from room contour.
	SQUARE_BY_ROOMCONTOUR = 0,
	// CUBOID: build a coarse "cuboid-like" contour by selecting wall top vertices first,
	// then run ContourSquare on that contour.
	SQUARE_BY_CUBOID = 1,
	// CONVEXITY: start from cuboid result, then apply convexity/defect-aware optimization.
	SQUARE_BY_CONVEXITY = 2,
	// MIN_LOSS: try multiple reference starts and choose minimum area-loss result.
	SQUARE_BY_MIN_LOSS = 3,
}eRoomSquare_type;
